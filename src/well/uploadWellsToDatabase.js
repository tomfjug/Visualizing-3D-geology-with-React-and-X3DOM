...
class UploadWellsDB extends React.Component {
    constructor(props) {
        super(props);
    ...
    updateData(result, fileNames, fileData) {
      ...
      for(let i = 0; i < fileData.length; i++){
        ...
        if(!found){
            ...
            axios.post('http://localhost:4000/rex3d/wells/add', well)
                        .then(res => {
    ...
    render(){
        return (
            <div>
                <input type="file" id="slice-file" multiple="multiple" name="files" onChange={(e) => {this.handleFile(e)}}/>
            </div>
        );
    }
}

export default UploadWellsDB;